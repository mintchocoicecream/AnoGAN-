...
class AnomalyDetectorEncoder(object):
	...
	def find_closest_match(self, test_data, test_data_name, test_batch_size=8):
		...
		new_res_loss = tf.reduce_sum(tf.abs(tf.subtract(test_data_tensor, closest_matches)), 1)
		new_dis_loss = tf.reduce_sum(tf.abs(tf.subtract(dis_f_input, dis_f_z)), 1)
		...
		samples = np.reshape(samples, (1, 128, 128))
		samples = (np.array(samples)+1)*127.5
		
#		errors = np.squeeze(errors)
		errors = np.reshape(errors, (1, 128, 128))
		...
		for i in range(samples.shape[0]):
			...
			if len(samples.shape) == 4:
				i_sample = samples[i,:,:,:]
				i_error = errors[1,:,:]
			f_res_loss = sess.run(new_res_loss)[i]
			f_dis_loss = sess.run(new_dis_loss)[i]
			print('f_res_loss:', f_res_loss, type(f_res_loss))
			print('f_dis_loss:', f_dis_loss, type(f_dis_loss))
			anomaly_score = (1-self.ano_para) * f_res_loss.sum() + (self.ano_para) * f_dis_loss.sum()
			print('anomaly_score:', anomaly_score)

			_path = self.test_data_folder
			path = _path + '/'+self.test_result_dir
			#path = os.path.join(_path, self.test_result_dir)
			if not os.path.isdir(path):
			  os.mkdir(path)
			#\\ -> / change			
			filename = ['AD_'+test_data_name[i].split("/")[-1], 
						'AD_error_'+str(anomaly_score)+'_'+test_data_name[i].split("/")[-1],
			...
			if i_sample.shape[2] == 3:
				cv2.imwrite(path+'/'+filename[0],cv2.cvtColor(i_sample, cv2.COLOR_RGB2BGR))
				#cv2.imwrite(os.path.join(path,filename[0]),cv2.cvtColor(i_sample, cv2.COLOR_RGB2BGR))
			else:
				cv2.imwrite(path+'/'+filename[0],i_sample)
				#cv2.imwrite(os.path.join(path,filename[0]),i_sample)
			cv2.imwrite(path+'/'+filename[1],i_error)
			#cv2.imwrite(os.path.join(path,filename[1]),i_error)
			np.save(path+'/'+filename[4],sess.run(latents)[i,:])
	...
	def get_test_data(self, folder):
		...
		tmp = []
		for item in self.test_data_names:
			tmp.append(item.replace('\\','/'))
		self.test_data_names=tmp
